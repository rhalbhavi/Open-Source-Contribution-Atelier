...
import { 
  ...
  Plus, Trash2, FileCode2, FileJson, FileType2 
} from 'lucide-react';
...
function getFileIcon(name: string) {
  if (name.endsWith('.ts') || name.endsWith('.tsx')) return <FileType2 size={16} className="text-blue-400" />;
  if (name.endsWith('.json')) return <FileJson size={16} className="text-yellow-400" />;
  if (name.endsWith('.js') || name.endsWith('.jsx')) return <FileCode2 size={16} className="text-yellow-300" />;
  return <FileIcon size={16} className="text-gray-400" />;
}

export function ProjectExplorer({ 
  files, activeFileId, onSelectFile, onCreateFile, onDeleteFile 
}: ProjectExplorerProps) {
  const [expandedFolders, setExpandedFolders] = useState<Set<string>>(new Set(['']));
  const [isCreating, setIsCreating] = useState<{ parentPath: string } | null>(null);
  ...
  const handleCreateSubmit = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter' && newFileName.trim()) {
      const fullPath = isCreating?.parentPath ? `${isCreating.parentPath}/${newFileName.trim()}` : newFileName.trim();
    ...
    return (
      <div key={node.path}>
        ...
        >
          ...
          <span className="mr-2">
            {node.type === 'folder' ? <Folder size={16} className="text-blue-400" /> : getFileIcon(node.name)}
          </span>
        ...
        {node.type === 'folder' && isExpanded && (
          <div>
            ...
            {isCreating?.parentPath === (node.path === 'root' ? '' : node.path) && (
              <div className="flex items-center py-1 pr-2" style={{ paddingLeft: `${(depth + 1) * 12 + 8}px` }}>
  ...
  return (
    <div className="flex flex-col h-full bg-[#1e1e1e] border-r border-gray-800 text-gray-300 w-64 flex-shrink-0">
      <div className="flex items-center justify-between p-3 border-b border-gray-800">
        <h3 className="text-xs font-bold uppercase tracking-wider text-gray-400">Project Files</h3>
        <button 
          onClick={() => {

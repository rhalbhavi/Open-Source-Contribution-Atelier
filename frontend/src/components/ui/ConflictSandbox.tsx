...
import { Check, AlertTriangle, GitMerge, ChevronRight, XCircle } from "lucide-react";
...
type BlockType = "normal" | "conflict";
    ...
    if (line.startsWith(">>>>>>>")) {
      if (inConflict) {
        ...
        if (currentConflict.currentContent?.endsWith("\n")) {
          currentConflict.currentContent = currentConflict.currentContent.slice(0, -1);
        }
        if (currentConflict.incomingContent?.endsWith("\n")) {
          currentConflict.incomingContent = currentConflict.incomingContent.slice(0, -1);
        }
...
export function ConflictSandbox({
  ...
  const [blocks, setBlocks] = useState<Block[]>(() => parseConflicts(initialContent));
  
  const allResolved = useMemo(() => {
    return blocks.every((b) => b.type === "normal" || b.resolution !== null);
  }, [blocks]);

  const handleResolve = (id: string, resolution: ConflictBlock["resolution"]) => {
    setBlocks((prev) =>
      prev.map((b) => {
  ...
  const handleComplete = () => {
    ...
    blocks.forEach((b) => {
      if (b.type === "normal") {
        ...
        else if (b.resolution === "incoming") finalString += b.incomingContent + "\n";
    ...
    <div className="flex flex-col border-4 border-black rounded-3xl bg-surface-lowest shadow-card overflow-hidden w-full max-w-5xl mx-auto dark:bg-[#151411] dark:border-[#2e2924]">
      ...
      <div className="bg-white border-b-4 border-black p-6 relative overflow-hidden dark:bg-[#1f1c18] dark:border-[#2e2924]">
        ...
        <p className="text-sm font-bold text-muted dark:text-[#c4bbae] max-w-xl">
          Git could not automatically merge your changes. Review the overlapping edits below and manually choose which lines to keep.
        </p>

        {/* Simplified Animated Timeline Graph */}
        <div className="mt-8 flex items-center justify-center relative h-24">
          <svg className="absolute inset-0 w-full h-full" overflow="visible">
             <path d="M 10 50 L 150 50" stroke="currentColor" strokeWidth="4" className="text-black dark:text-[#2e2924]" fill="none" />
             <path d="M 50 50 C 80 10, 100 10, 120 10 L 250 10" stroke="#FF3B30" strokeWidth="4" fill="none" strokeDasharray="6 6" className="animate-[dash_1s_linear_infinite]" />
             <path d="M 150 50 L 250 50" stroke="currentColor" strokeWidth="4" className="text-black dark:text-[#2e2924]" fill="none" />
             <path d="M 250 10 C 270 10, 290 50, 310 50 L 350 50" stroke="#FF3B30" strokeWidth="4" fill="none" />
             
             {/* Nodes */}
             <circle cx="50" cy="50" r="8" fill="currentColor" className="text-black dark:text-[#f0ebe2]" />
             <circle cx="150" cy="50" r="8" fill="currentColor" className="text-black dark:text-[#f0ebe2]" />
             <circle cx="120" cy="10" r="8" fill="#FF3B30" />
             <circle cx="250" cy="10" r="8" fill="#FF3B30" />
             <circle cx="250" cy="50" r="8" fill="currentColor" className="text-black dark:text-[#f0ebe2]" />
             
             {/* Conflict Node */}
             <circle cx="310" cy="50" r="14" fill="#FFD600" stroke="#000" strokeWidth="4" className="animate-pulse" />
             <text x="310" y="55" textAnchor="middle" fontSize="16" fontWeight="900" fill="#000">!</text>
             
             {/* Labels */}
             <text x="50" y="75" fontSize="12" fontWeight="bold" fill="currentColor" className="text-muted dark:text-[#c4bbae]">Base</text>
             <text x="120" y="-5" fontSize="12" fontWeight="bold" fill="#FF3B30">{featureBranchName}</text>
             <text x="150" y="75" fontSize="12" fontWeight="bold" fill="currentColor" className="text-muted dark:text-[#c4bbae]">{baseBranchName}</text>
             <text x="310" y="80" fontSize="12" fontWeight="bold" fill="#FFD600">Conflict</text>
          </svg>
        </div>
      </div>
      ...
      <div className="bg-surface-lowest p-6 dark:bg-[#0f0e0c] font-mono text-sm leading-relaxed overflow-x-auto">
        {blocks.map((block, idx) => {
          if (block.type === "normal") {
            return (
              <div key={idx} className="py-2 px-4 text-text/80 dark:text-[#c4bbae]/80 whitespace-pre">
                {block.content}
              </div>
            );
          }
          ...
          return (
            ...
            >
              {/* Conflict Header */}
              <div className={`p-3 border-b-4 border-inherit flex items-center justify-between bg-inherit ${isResolved ? "bg-green-100 dark:bg-green-900/30" : "bg-yellow-50 dark:bg-yellow-900/30"}`}>
                <div className="flex items-center gap-2 font-black uppercase text-xs tracking-widest">
                  {isResolved ? (
                    <span className="text-green-700 dark:text-green-400 flex items-center gap-1"><Check size={16}/> Resolved</span>
                  ) : (
                    <span className="text-yellow-700 dark:text-yellow-400 flex items-center gap-1"><AlertTriangle size={16}/> Unresolved Conflict</span>
                  )}
                </div>
                {isResolved && (
                  <button
              ...
              <div className="flex flex-col md:flex-row divide-y-4 md:divide-y-0 md:divide-x-4 divide-inherit bg-white dark:bg-[#151411]">
                ...
                <div className={`flex-1 flex flex-col ${isResolved && block.resolution !== "current" && block.resolution !== "both" ? "opacity-30 grayscale pointer-events-none" : ""}`}>
                ...
                <div className={`flex-1 flex flex-col ${isResolved && block.resolution !== "incoming" && block.resolution !== "both" ? "opacity-30 grayscale pointer-events-none" : ""}`}>

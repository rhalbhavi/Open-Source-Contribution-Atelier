    ...
    <div className="w-80 h-full bg-white dark:bg-[#1a1a1a] border-l border-gray-200 dark:border-gray-800 flex flex-col shadow-lg z-10">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {!activeThread ? (
          ...
          activeThread.comments.map((comment) => (
            <div
              key={comment.id}
              className="bg-gray-50 dark:bg-[#252525] p-3 rounded-lg border border-gray-100 dark:border-gray-800"
            >
      ...
      <div className="p-4 border-t border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-[#1e1e1e]">
        <form onSubmit={handleSubmit} className="flex gap-2">
          <div className="flex-1 bg-white dark:bg-[#252525] border border-gray-200 dark:border-gray-700 rounded-md">
            <MentionTextarea
              value={newComment}
              onChange={(value) => setNewComment(value)}
              placeholder="Type a comment... (use @ to mention)"
              className="text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 max-h-32"
              rows={2}
            />
          </div>
          <button

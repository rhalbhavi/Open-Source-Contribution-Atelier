...
export function MergeConflictSandbox() {
  const [resolvedStatus, setResolvedStatus] = useState<"unresolved" | "current" | "incoming" | "both">("unresolved");
    ...
    <div className="w-full rounded-3xl border-4 border-black bg-white shadow-card overflow-hidden dark:bg-[#1f1c18] dark:border-[#2e2924] flex flex-col my-8">
      ...
      <div className="p-6 md:p-8 space-y-8">
        ...
        <div className="relative pt-6 pb-2">
          ...
          {/* Merge line */}
          <svg className="absolute top-[30%] left-[55%] w-[10%] h-[40%] text-green-500 overflow-visible" preserveAspectRatio="none">
            <path d="M 0,100 C 50,100 50,0 100,0" fill="none" stroke="currentColor" strokeWidth="4" />
          </svg>
          ...
          <div className="flex justify-between relative z-10">
            ...
            <div className="flex flex-col items-center gap-2 mt-8 group">
              ...
              <span className="text-[10px] font-black font-mono">Feat start</span>
            </div>
            <div className="flex flex-col items-center gap-2 group">
              <div className="w-6 h-6 rounded-full border-4 border-black bg-blue-500 shadow-card-sm group-hover:scale-125 transition-transform" />
              <span className="text-[10px] font-black font-mono">Main update</span>
            </div>
            <div className="flex flex-col items-center gap-2 mt-8 group">
              <div className="w-6 h-6 rounded-full border-4 border-black bg-green-500 shadow-card-sm group-hover:scale-125 transition-transform" />
              <span className="text-[10px] font-black font-mono">Feat update</span>
            </div>
            <div className="flex flex-col items-center gap-2 group relative">
              <div className={`w-8 h-8 rounded-full border-4 border-black shadow-card-sm flex items-center justify-center transition-all duration-500 ${isMerged ? 'bg-green-400' : 'bg-red-500 animate-pulse'}`}>
                {isMerged ? <Check size={16} className="text-black" /> : <AlertTriangle size={16} className="text-white" />}
              </div>
        ...
        <div className="space-y-4">
          ...
          {!isMerged ? (
              ...
              <div className="rounded-xl border-4 border-black overflow-hidden shadow-card-sm bg-[#1e1e1e] font-mono text-sm">
                ...
                <div className="p-4 space-y-1">
                  ...
                  <div className="my-4 border-l-4 border-red-500 pl-2 bg-red-500/10 rounded-r">
                    ...
                    <div className={`mt-2 p-2 font-bold ${resolvedStatus === "current" || resolvedStatus === "both" ? "text-blue-400 bg-blue-900/30" : "text-white/40"}`}>
                      {"<<<<<<< HEAD (Current Change)"}
                      <br/>
                      {"  const fetchUsers = () => api.get('/v2/users');"}
                    </div>

                    <div className="text-red-400 font-black py-1">{"======="}</div>
                    ...
                    <div className={`mt-2 p-2 font-bold ${resolvedStatus === "incoming" || resolvedStatus === "both" ? "text-green-400 bg-green-900/30" : "text-white/40"}`}>
                ...
                <div className="flex-1 bg-surface-lowest border-4 border-black rounded-xl p-6 shadow-card-sm flex flex-col justify-center items-center text-center gap-4 dark:bg-[#151411] dark:border-[#2e2924]">
                  ...
                  {resolvedStatus === "unresolved" ? (
                    ...
                    <div className="w-full text-left font-mono text-sm bg-[#1e1e1e] text-white p-4 rounded-xl border-4 border-black shadow-inner">
                      <div className="text-white/50">{"function init() {"}</div>
                      {(resolvedStatus === "current" || resolvedStatus === "both") && (
                        <div className="text-blue-400 font-bold pl-4">{"const fetchUsers = () => api.get('/v2/users');"}</div>
                      )}
                      {(resolvedStatus === "incoming" || resolvedStatus === "both") && (
                        <div className="text-green-400 font-bold pl-4">{"const fetchUsers = () => api.get('/v3/users', { timeout: 5000 });"}</div>
                      )}
                      <div className="text-white/50">{"}"}</div>
                    </div>
                  )}
            ...
            <div className="bg-green-100 border-4 border-green-600 rounded-2xl p-8 flex flex-col items-center justify-center text-center gap-4 animate-in fade-in zoom-in duration-500">
              ...
              <h4 className="text-3xl font-black text-green-900 drop-shadow-sm">Conflict Resolved!</h4>
              <p className="text-green-800 font-bold max-w-md">
                You successfully evaluated the overlapping changes and merged the code. 
                In a real repository, you would now commit this resolution.
              </p>
              <div className="mt-4 text-left font-mono text-sm bg-black text-white p-6 rounded-xl border-4 border-black w-full max-w-lg shadow-inner">
                <div className="text-green-400 font-black mb-2 border-b border-white/20 pb-2">Output code:</div>
                <div className="text-white/50">{"function init() {"}</div>
                {(resolvedStatus === "current" || resolvedStatus === "both") && (
                  <div className="text-white font-bold pl-4">{"const fetchUsers = () => api.get('/v2/users');"}</div>
                )}
                {(resolvedStatus === "incoming" || resolvedStatus === "both") && (
                  <div className="text-white font-bold pl-4">{"const fetchUsers = () => api.get('/v3/users', { timeout: 5000 });"}</div>
                )}
                <div className="text-white/50">{"}"}</div>
              </div>
            </div>

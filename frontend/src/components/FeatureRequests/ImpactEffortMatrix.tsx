...
export const ImpactEffortMatrix: React.FC<ImpactEffortMatrixProps> = ({ features }) => {
  ...
  return (
    <div className="bg-dark-800/50 p-6 rounded-xl border border-dark-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {(Object.entries(matrixData) as [keyof typeof matrixData, FeatureRequest[]][]).map(([key, items]) => (
          <div
            key={key}
          ...
          >
            ...
            <div className="space-y-1">
              {items.slice(0, 5).map((feature) => (
                <div key={feature.id} className="text-sm text-gray-400 truncate">
                  • {feature.title}
              ...
              {items.length > 5 && (
                <div className="text-xs text-gray-500">+{items.length - 5} more</div>
              )}
            </div>
          </div>
        ))}

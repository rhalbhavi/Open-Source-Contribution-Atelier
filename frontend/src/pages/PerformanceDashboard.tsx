...
const MOCK_API_LATENCY = [
  {
    ...
    p50: 120,
    p95: 310,
    budget: budget.api.endpoints[0].p95Ms,
  },
  {
    endpoint: "/api/dashboard/",
    p50: 95,
    p95: 280,
    budget: budget.api.endpoints[1].p95Ms,
  },
  {
    endpoint: "/api/leaderboard/",
    p50: 80,
    p95: 210,
    budget: budget.api.endpoints[2].p95Ms,
  },
  {
    endpoint: "/api/auth/me/",
    p50: 45,
    p95: 110,
    budget: budget.api.endpoints[3].p95Ms,
  },
  {
    endpoint: "/api/progress/",
    p50: 130,
    p95: 340,
    budget: budget.api.endpoints[4].p95Ms,
  },
];
...
export default function PerformanceDashboard() {
  const [activeTab, setActiveTab] = useState<"bundle" | "lighthouse" | "api">(
    "bundle",
  );
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <h1 className="text-3xl font-black mb-1 text-text">
        ⚡ Performance Dashboard
      </h1>
      <p className="text-muted mb-6">
        Real-time view of performance budgets enforced in CI. Budgets defined in{" "}
        <code className="bg-surface-low px-1 rounded text-sm">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <div
          className={`rounded-xl border-2 p-4 ${bundleStatus.js ? "border-green-500" : "border-red-500"}`}
        >
          <p className="text-xs font-bold uppercase tracking-widest text-muted mb-1">
            JS Bundle
          </p>
          <p
            className={`text-3xl font-black ${bundleStatus.js ? "text-green-600" : "text-red-600"}`}
          >
            {currentBundle.jsKB}KB
          </p>
          <p className="text-xs text-muted">
            Budget: {budget.bundle.totalJsGzipKB}KB
          </p>
        </div>
        <div
          className={`rounded-xl border-2 p-4 ${bundleStatus.css ? "border-green-500" : "border-red-500"}`}
        >
          <p className="text-xs font-bold uppercase tracking-widest text-muted mb-1">
            CSS Bundle
          </p>
          <p
            className={`text-3xl font-black ${bundleStatus.css ? "text-green-600" : "text-red-600"}`}
          >
            {currentBundle.cssKB}KB
          </p>
          <p className="text-xs text-muted">
            Budget: {budget.bundle.totalCssGzipKB}KB
          </p>
        </div>
        <div
          className={`rounded-xl border-2 p-4 ${lhScores.performance >= budget.lighthouse.performance ? "border-green-500" : "border-red-500"}`}
        >
          <p className="text-xs font-bold uppercase tracking-widest text-muted mb-1">
            LH Performance
          </p>
          <p
            className={`text-3xl font-black ${lhScores.performance >= budget.lighthouse.performance ? "text-green-600" : "text-red-600"}`}
          >
            {lhScores.performance}
          </p>
          <p className="text-xs text-muted">
            Budget: ≥ {budget.lighthouse.performance}
          </p>
        </div>
        <div
          className={`rounded-xl border-2 p-4 ${lhScores.accessibility >= budget.lighthouse.accessibility ? "border-green-500" : "border-red-500"}`}
        >
          <p className="text-xs font-bold uppercase tracking-widest text-muted mb-1">
            LH Accessibility
          </p>
          <p
            className={`text-3xl font-black ${lhScores.accessibility >= budget.lighthouse.accessibility ? "text-green-600" : "text-red-600"}`}
          >
            {lhScores.accessibility}
          </p>
          <p className="text-xs text-muted">
            Budget: ≥ {budget.lighthouse.accessibility}
          </p>
        </div>
      </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {LIGHTHOUSE_CATEGORIES.map((cat) => {
              ...
              return (
                <div
                  key={cat}
                  className={`rounded-xl p-4 border-2 text-center ${passing ? "border-green-500 bg-green-50" : "border-red-500 bg-red-50"}`}
                >
                  <p className="text-xs font-bold uppercase tracking-widest text-muted mb-2 capitalize">
                    {cat}
                  </p>
                  <p
                    className={`text-4xl font-black ${passing ? "text-green-700" : "text-red-700"}`}
                  >
                    {score}
                  </p>
            ...
            <BarChart data={MOCK_API_LATENCY} layout="vertical">
              ...
              <Bar dataKey="p95" name="p95">
                {MOCK_API_LATENCY.map((entry, i) => (
                  <Cell
                    key={i}
                    fill={entry.p95 <= entry.budget ? "#22c55e" : "#ef4444"}
                  />
                ))}
              </Bar>
            </BarChart>

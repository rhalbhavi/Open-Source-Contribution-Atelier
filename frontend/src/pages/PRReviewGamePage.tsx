...
const LEVELS = [
  {
    ...
    description: "Review this pull request and find the bug. Should you approve, request changes, or comment?",
    originalCode: `function countDown(start) {\n  let current = start;\n  while (current > 0) {\n    console.log(current);\n    current--;\n  }\n}`,
    modifiedCode: `function countDown(start) {\n  let current = start;\n  while (current > 0) {\n    console.log(current);\n    // bug introduced here\n    current++;\n  }\n}`,
    correctAction: "request_changes",
    feedback: "Good catch! The developer changed `current--` to `current++`, creating an infinite loop.",
  },
  {
    id: 2,
    ...
    feedback: "Nice! Leaving a comment is best here. It's not necessarily broken, but we usually shouldn't merge console.logs into production.",
  },
  {
    id: 3,
    ...
    feedback: "Perfect! You approved a great optimization that uses built-in localeCompare.",
  }
];
...
export function PRReviewGamePage() {
  ...
  const handleAction = (action: "approve" | "request_changes" | "comment") => {
    ...
    if (action === level.correctAction) {
      ...
      toast.error(`Not quite! The best action was to ${level.correctAction.replace("_", " ")}.`, { duration: 4000 });
    }
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-6 flex flex-col min-h-[calc(100vh-64px)]">
      <div>
        ...
        <p className="mt-2 text-muted dark:text-[#c4bbae]">
          Test your code review skills. Analyze the diffs and choose the right action!
        </p>
      </div>
      ...
      <div className="flex items-center justify-between bg-surface-low border-2 border-black rounded-xl p-4 dark:bg-[#151411] dark:border-[#2e2924]">
        <span className="font-bold text-lg dark:text-[#f0ebe2]">Score: <span className="text-primary">{score}</span></span>
        {!gameOver && (
          <span className="font-bold text-lg dark:text-[#f0ebe2]">Level {currentLevel + 1} of {LEVELS.length}</span>
        )}
      </div>
      ...
      {!gameOver ? (
        <div className="flex-1 flex flex-col gap-6">
          <div className="bg-white dark:bg-[#1f1c18] border-4 border-black dark:border-[#2e2924] rounded-2xl p-6 shadow-card">
            <h2 className="text-xl font-black mb-2 dark:text-[#f0ebe2]">{LEVELS[currentLevel].title}</h2>
            <p className="text-muted dark:text-[#c4bbae] mb-6">{LEVELS[currentLevel].description}</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            >
              ...
              <span className="text-xs text-green-800 text-center">Looks good to me! Merge it.</span>
            </button>
            ...
            >
              ...
              <span className="text-xs text-blue-800 text-center">Leave feedback without blocking.</span>
            </button>
            ...
            >
              ...
              <span className="text-xs text-red-800 text-center">Found a bug! Block merge.</span>
            </button>
          </div>
        </div>
      ) : (
        <div className="flex-1 flex flex-col items-center justify-center bg-white dark:bg-[#1f1c18] border-4 border-black dark:border-[#2e2924] rounded-2xl p-12 shadow-card text-center gap-6">
          ...
          <h2 className="text-4xl font-black dark:text-[#f0ebe2]">Game Over!</h2>
          <p className="text-xl text-muted dark:text-[#c4bbae]">Your final review score is <span className="text-primary font-black">{score}</span> / {LEVELS.length * 100}</p>
          <button
            onClick={resetGame}

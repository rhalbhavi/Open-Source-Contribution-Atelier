...
class PaginationValidator:
    ...
    def validate_pagination_structure(self, response_data: Dict) -> bool:
        ...
        if not isinstance(response_data.get('count'), int):
            return False
        if response_data.get('next') is not None and not isinstance(response_data.get('next'), str):
            return False
        if response_data.get('previous') is not None and not isinstance(response_data.get('previous'), str):
            return False
    ...
    def _is_list_endpoint(self, url: str) -> bool:
        ...
        list_indicators = ['list', 'all', 'search', 'feed', 'timeline', 'badges', 'users']

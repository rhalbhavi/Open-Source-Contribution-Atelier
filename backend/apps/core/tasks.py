...
def _purge_for_model(model, threshold_date, batch_size):
    ...
    logger.info(f"Found {total_to_delete} expired records for {model.__name__}. Beginning purge.")
    deleted_count = 0
    ...
    logger.info(f"Successfully purged {deleted_count} records from {model.__name__} in {duration:.2f}s")

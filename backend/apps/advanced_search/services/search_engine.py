...
class AdvancedSearchEngine:
    ...
    def search(
        self, query: str, user, filters: Dict = None, limit: int = 20, offset: int = 0
    ) -> Tuple[List[Dict], int]:
        """
    ...
    def _search_content(
        self, processed_query: Dict, filters: Dict, limit: int
    ) -> Tuple[List[Dict], int]:
        """
    ...
    def _calculate_interest_score(
        self, result: Dict, profile: UserSearchProfile
    ) -> float:
        """
    ...
    def _merge_results(
        self, base_results: List[Dict], semantic_results: List[Dict]
    ) -> List[Dict]:
        """
        ...
        existing = {
            f"{r.get('content_type')}_{r.get('object_id')}": r for r in base_results
        }
    ...
    def _personalize_results(
        self, results: List[Dict], user, profile: Optional[UserSearchProfile]
    ) -> List[Dict]:
        """
        ...
        for result in results:
            ...
            if result_id and result_id in profile.clicked_results:
                result["relevance_score"] = min(
                    result.get("relevance_score", 0) + 0.2, 1.0
                )
    ...
    def _get_filter_suggestions(
        ...
        suggestions["difficulties"] = sorted(
            difficulties.items(), key=lambda x: x[1], reverse=True
        )
        ...
        suggestions["tags"] = sorted(tags.items(), key=lambda x: x[1], reverse=True)[
            :10
        ]
        ...
        if user and user.is_authenticated:
            try:
                ...
                profile.add_search_query(
                    query, results[:10], [r.get("id") for r in results[:5]]
                )
            except UserSearchProfile.DoesNotExist:
                pass

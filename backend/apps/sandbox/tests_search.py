...
from apps.sandbox.models import Project, ProjectFile, BulkReplaceOperation
...
def setup_files(project):
    f1 = ProjectFile.objects.create(project=project, path="src/main.py", content="def test():\n    print('hello world')")
    f2 = ProjectFile.objects.create(project=project, path="src/utils.py", content="def log():\n    print('hello world')")
    return [f1, f2]
...
def test_global_replace(api_client, user, project, setup_files):
    ...
    # Valid replace
    response = api_client.post(url, {"query": "hello world", "replacement": "hi universe"})

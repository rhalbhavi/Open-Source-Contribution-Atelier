...
class LessonPluginRegistry:
    ...
    def register(self, plugin_class: Type[LessonPlugin]):
        if not issubclass(plugin_class, LessonPlugin):
            logger.error(f"Failed to load plugin: {plugin_class} must inherit from LessonPlugin")
            return
            
        identifier = plugin_class.identifier
        if not identifier:
            logger.error(f"Failed to load plugin: {plugin_class} is missing an identifier")
            return
            
        self._plugins[identifier] = plugin_class
        logger.info(f"Successfully registered lesson plugin: {identifier} (v{plugin_class.version})")

...
from apps.progress.models import XPEvent, LessonProgress, UserBadge, Badge, StreakProfile
...
class WeeklyDigestServiceTests(TestCase):
    def setUp(self):
        ...
        # Create badge
        self.badge = Badge.objects.create(name="First Steps", slug="first-steps", description="First step.")
    ...
    def test_digest_with_no_activity(self):
        ...
        self.assertEqual(len(context["new_content"]), 4) # Created all 4 lessons recently in setup
        self.assertEqual(len(context["recommendations"]), 3)

    def test_digest_with_recent_activity(self):
        # 1. Add XP
        XPEvent.objects.create(user=self.user, source_type="lesson", base_points=10, xp_delta=10)
        XPEvent.objects.create(user=self.user, source_type="exercise", base_points=20, xp_delta=20)
        
        # Manually alter created_at for older event (cannot do easily in create because of auto_now_add)
        old_xp = XPEvent.objects.create(user=self.user, source_type="lesson", base_points=50, xp_delta=50)
        ...
        old_lp = LessonProgress.objects.create(user=self.user, lesson=self.lesson2, completed=True)
        ...
        old_ub = UserBadge.objects.create(user=self.user, badge=Badge.objects.create(name="Old Badge", slug="old"))

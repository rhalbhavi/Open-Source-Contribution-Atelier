...
class SecureAccountDeleteTest(TestCase):
    def setUp(self):
        ...
        self.user = User.objects.create_user(username='john_doe', email='john@example.com', password='pass')
        self.lesson = Lesson.objects.create(title='Test', slug='test', content='content')
        
        # PII data
        Note.objects.create(user=self.user, title='My Note', encrypted_content='enc', iv='iv')
        Certificate.objects.create(user=self.user, course_name='Open Source')
        
        # Public data
        Comment.objects.create(user=self.user, lesson=self.lesson, content='Great lesson!')
    ...
    def test_deletion_with_no_contributions_or_pii(self):
        ...
        empty_user = User.objects.create_user(username='ghost', email='ghost@example.com', password='pass')

...
for dup, orig in duplicates_to_close:
    ...
    comment_data = json.dumps(
        {"body": f"Closing this as it is a duplicate of #{orig['number']}."}
    ).encode("utf-8")
    ...
    close_data = json.dumps({"state": "closed", "state_reason": "not_planned"}).encode(
        "utf-8"
    )
    req = urllib.request.Request(
        close_url, data=close_data, headers=headers, method="PATCH"
    )
    try:
        with urllib.request.urlopen(req, context=ctx):

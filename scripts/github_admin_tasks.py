...
def review_merge_prs(repo):
    ...
    for pr in prs:
        ...
        requests.post(
            ...
            headers=headers,
            json={
                "body": "Automated Review: PR looks good and complies with ssoc26 guidelines. Merging!"
            },
        )
        ...
        merge_response = requests.put(
            merge_url,
            headers=headers,
            json={"commit_title": f"Auto-merge PR #{pr_number}"},
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--repo", default=DEFAULT_REPO, help="Repository in format owner/repo"
    )
    subparsers = parser.add_subparsers(dest="command", required=True)

    subparsers.add_parser(
        "sync-pr-labels",
        help="Add ssoc26 to all PRs, copy easy/medium/hard from linked issues",
    )
    subparsers.add_parser("sync-issue-labels", help="Add ssoc26 to all issues")

    create_parser = subparsers.add_parser(
        "create-issues", help="Create new development issues"
    )
    create_parser.add_argument(
        "--count", type=int, default=50, help="Number of issues to create (default: 50)"
    )

    subparsers.add_parser(
        "review-merge-prs", help="Add a review comment and merge open PRs"
    )
